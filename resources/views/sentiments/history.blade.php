...
@section('content')
    ...
    <div class="glass-card card-hover rounded-xl overflow-hidden border-none shadow-xl">
        <div class="bg-gradient-to-r from-blue-500 to-cyan-600 text-white p-6">
            ...
            <p class="text-white/80">Data sentimen pelanggan yang telah Anda analisis.</p>
        </div>
        <div class="p-6">
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                @if(count($sentiments) > 0)
                    <div class="flex gap-2">
                        <a href="{{ route('sentiments.print') }}" target="_blank"
                           class="inline-flex items-center gap-1 px-3 py-1.5 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <a href="{{ route('sentiments.export') }}"
                           class="inline-flex items-center gap-1 px-3 py-1.5 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            @foreach($sentiments as $index => $sentiment)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $badgeClass = $sentiment->sentiment === 'positive'
                                                ? 'bg-green-500'
                                                : ($sentiment->sentiment === 'negative'
                                                    ? 'bg-red-500'
                                                    : 'bg-yellow-500');
                                            $sentimentText = ucfirst($sentiment->sentiment);
                                        @endphp
                                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold text-white {{ $badgeClass }}">
                                            {{ $sentimentText }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            <div class="w-16 bg-gray-200 rounded-full h-2">
                                                @php
                                                    $barClass = $sentiment->sentiment === 'positive'
                                                        ? 'bg-green-500'
                                                        : ($sentiment->sentiment === 'negative'
                                                            ? 'bg-red-500'
                                                            : 'bg-yellow-500');
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('sentiments.edit', $sentiment->id) }}"
                                               class="p-1 rounded-full hover:bg-blue-100 hover:text-blue-600"
                                               title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            ...
                                            <form action="{{ route('sentiments.destroy', $sentiment->id) }}" method="POST" class="inline-block">
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                        class="p-1 rounded-full hover:bg-red-100 hover:text-red-600"
            ...
            <!-- Impor Data -->
            <div class="mt-8 p-4 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg">
                ...
                <form action="{{ route('sentiments.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <p class="text-sm text-white/70 mt-2">Format yang didukung: CSV dan JSON. Kolom yang diperlukan: text, sentiment (optional: probability).</p>
                </form>
            </div>
